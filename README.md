...
每个文档站有独立的 `zensical.toml` 与 `docs/`；开发服务器只提供其中一个站点，配置生成器使用上面的 Vite 服务器单独运行。新增文档站时，新建 `<项目>/zensical.toml` 与 `<项目>/docs/`，并加入 `scripts/build-site.py` 的站点列表和门户链接。
...
浏览器测试使用独立的 npm 清单和锁文件，测试依赖不打包进应用。启动上面的组装预览后运行（本地默认使用已安装的 Edge）：
...
| `scripts/build-site.py` | 构建全部文档站与生成器并组装到 `site/`，不发布 |
| `tests/config-generator/` | 独立解析器、真实 TUIC、站点与浏览器检查 |
| `.github/workflows/deploy.yml` | GitHub Pages 构建与发布流程 |

只维护简体中文。更新 TUIC 或 Wind 后，核对生成器的版本基线及字段的实际运行行为，不把尚未接入客户端运行逻辑的配置暴露成可用功能。示例使用占位域名与运行时生成的测试凭据，不加入真实部署数据。
