"""Check assembled site links and the standalone editor's privacy/assets."""
...
editor_path = root / 'config-editor/index.html'
editor = editor_path.read_text(encoding='utf-8')
for marker in ('googletagmanager', 'google-analytics', 'gtag('):
    assert marker not in editor, 'Editor must not include third-party analytics'
for marker in ('md-header', 'md-main', 'iframe', 'app.mjs'):
    assert marker not in editor, 'Editor must be a standalone Svelte/WASM application'
assets = re.findall(r'''(?:src|href)=["']([^"']+\.(?:js|wasm|css))["']''', editor)
assert any(asset.endswith('.js') for asset in assets), 'Application entry missing'
wasm_assets = list((root / 'config-editor/assets').glob('*.wasm'))
assert wasm_assets, 'Rust WASM engine missing'
bundles = list((root / 'config-editor/assets').glob('*.js'))
...
for asset in assets:
    url = urlsplit(asset)
    assert not url.scheme and not url.netloc, 'Editor must use local assets'
    target = root / url.path.lstrip('/') if url.path.startswith('/') else editor_path.parent / url.path
    assert target.is_file(), f'Missing editor asset: {asset}'
assert not errors, '\n'.join(errors)
