...
try {
  ...
  await click('配置详解');
  assert.equal(await page.getByRole('heading', { name: '配置项详解' }).count(), 1);
  ...
  assert.ok((await page.locator('.cg-desc-line').allTextContents()).some(line => line.includes('auto_ssl: true')));
  ...
  await page.screenshot({ path: resolve('.cache/config-description-mobile.png'), fullPage: true });
