...
<aside id="ce-preview" class="ce-output" aria-label="配置预览区" tabindex="-1">
  <div class="ce-output-top">
    <h2>配置预览</h2>
    <span class="ce-validity" role="status" data-valid={String(view.valid)}>
      {view.valid ? '可导出' : `${errors.length} 项待填写或修正`}
    </span>
  </div>
  <div class="ce-toolbar">
    <div class="ce-sides" role="group" aria-label="配置预览类型">
  ...
  <details class="ce-errors" hidden={!errors.length} open>
  ...
  {:else}
    <div class="ce-code"><code>无法生成预览，请检查配置。</code></div>
  {/if}
  <ExportActions />
  <div class="ce-export-notes"><p class="ce-hint">{view.ui.export_hint}</p>{#if view.command}<code class="ce-command">{view.command}</code>{/if}<Notices notices={view.notices} /></div>
</aside>
