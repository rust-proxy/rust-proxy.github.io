...
<div class="ce-user" hidden={!row.visible} data-row={row.id}>
  <div class="ce-row-title">
    ...
    <div class="ce-row-actions">
      ...
      {#if collection.removable}<button class="ce-remove" type="button" aria-label={`移除${collection.label} ${row.number}`} onclick={() => onremove(row.id)}>移除</button>{/if}
    </div>
  </div>
  <div class="ce-fields">
