//! Typed legacy fixtures are a test oracle, never part of the editor runtime.
...
pub fn build_configs(s: &State) -> Result<Value, String> {
	config_editor::model::build_configs(document().map_err(|e| e.to_string())?, &s.value())
}
pub fn redact_configs(v: &Value) -> Result<Value, String> {
	document().map_err(|e| e.to_string())?.redact(v).map_err(|e| e.to_string())
}
pub fn validate(s: &State) -> config_editor::validation::Errors {
